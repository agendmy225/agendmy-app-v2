...
const checkIfUserIsOwner = async (): Promise<boolean> => {
  try {
    ...
    console.error('âŒ [checkIfUserIsOwner] Erro ao verificar tipo de usuário:', error);
    return false;
  }
};
...
const testFirestoreConnection = async (): Promise<boolean> => {
  try {
    ...
    const currentUser = firebaseAuth.currentUser;
    if (!currentUser) {
      console.error('âŒ [testFirestore] Usuário não autenticado');
      return false;
    }
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load service data  
        console.log('ðŸ› ï¸ [BookingConfirmation] Carregando dados do serviço...');
        ...
        if (businessData) {
          ...
          console.error('âŒ [BookingConfirmation] Documento do estabelecimento não encontrado');
          setBusinessData({ id: businessId, name: 'Estabelecimento', address: 'Endereço indisponível', imageUrl: '' } as Business);
        }
        ...
        if (serviceData) {
          ...
          console.error('âŒ [BookingConfirmation] Documento do serviço não encontrado');
          setServiceData({ id: serviceId, name: 'Serviço', price: 0, duration: '60min' } as Service);
        }
        ...
        if (professionalData) {
          ...
          console.error('âŒ [BookingConfirmation] Documento do profissional não encontrado');
          setProfessionalData({ id: professionalId, name: 'Profissional' } as Professional);
        }
